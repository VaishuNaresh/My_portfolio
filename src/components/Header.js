...
export default function Header() {
    ...
    return <header className="flex justify-between px-8 py-3 bg-primary sticky top-0 z-50 glass-header">
        <a className="font-bold text-[#fe7b9a] text-lg">Vaishali</a>
        ...
        {toggleMenu && <nav className="block md:hidden">
            <ul onClick={() => setToggleMenu(!toggleMenu)} className="flex flex-col text-gray-600 mobile-nav">
                <li><a>Home</a></li>
        ...
        <button onClick={() => setToggleMenu(!toggleMenu)} className='block md:hidden'><Bars3Icon className='text-gray-500 h-5'/></button>
    </header>
}